...
def run_pipeline(device, host, port):
    pipeline_str = (
        ...
        # 소스: 드라이버가 60fps만 지원한다고 우기므로, 60/1로 받아줍니다.
        # 하지만 하드웨어(frame_sync=1) 때문에 실제 데이터는 30fps로 들어옵니다.
        "video/x-raw(memory:NVMM),format=UYVY,width=1920,height=1200,framerate=60/1 ! "
        
        # videorate: 들어오는 실제 프레임 속도에 맞춰 타임스탬프를 30fps로 재조정합니다.
